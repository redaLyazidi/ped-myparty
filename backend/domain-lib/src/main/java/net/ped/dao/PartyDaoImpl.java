...
public class PartyDaoImpl extends GenericDAO implements InterfacePartyDao {
	...
	public List<Artist> getAllArtists() throws Exception {
		...
		try {
			...
			tx.commit();
			LOG.debug("la recherche a reussi, taille du resultat : "
					+ list.size());
	...
	public List<Party> getAllPartiesNotValidated() {
		...
		try {
			...
			tx.begin();
			list = em.createQuery(
					"from Party u inner join fetch u.artists "
	...
	public int getNbPartiesCriteria(String place, Double priceBegin, Double priceEnd, Calendar date, Calendar time) {
		...
		try {
			...
			LOG.error("Erreur dans la fonction getNbPartiesCriteria", re);
			throw re;
		} finally {
			closeEntityManager();
		}
